...
if args.net=='res18':
    ...
    from models.monarch_vit_small import ViT
    net = ViT(
    image_size = size,
    patch_size = args.patch,
    num_classes = 10,
...
def test(epoch):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/'+args.net+'-{}-ckpt.pth'.format(args.patch))
        best_acc = acc
...
print('Mean Dataloader time:', counter/100)
print('Mean Training time:', training_counter/100)
